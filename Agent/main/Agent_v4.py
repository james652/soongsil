...
def build_report(steps: List[StepResult], gpt_model: str = "gpt-4o") -> Dict[str, Any]:
...
def analyze_only(specs: List[StepSpec], gpt_model: str = "gpt-4o") -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    choice = input("어떤 프로그램을 실행할까요? (Brainwash / Accumulative / Test / Analysis) [Brainwash]: ").strip() or "Brainwash"

    if choice.lower() in ("analysis", "analyze", "a"):
        # 분석 스펙 불러오기
        specs = build_analyze_brainwash_specs()
        # 분석 수행
        analysis_report = analyze_only(specs, gpt_model="gpt-4o")
        ...
        out = monitor_pipeline(specs, use_gpt=True, gpt_model="gpt-4o")
        report = build_report(out, gpt_model="gpt-4o")
        print("\n" + "=" * 40)
