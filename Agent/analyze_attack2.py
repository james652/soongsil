...
final_prompt = ChatPromptTemplate.from_messages([
    ("system", "너는 머신러닝 보안 전문가야. 제공된 모든 정보를 종합해 **4단계 결과**를 반드시 **유효한 JSON 하나**로만 출력하고, 한국어로 답하라."),
    MessagesPlaceholder(variable_name="chat_history"),
    ("user", """
[정확도 변화]
- 공격 전: {before_acc}
...
def analyze_attack(before_acc, after_acc, previous_output_log="", session_id="attack-session") -> Tuple[Any, Dict[str, Any]]:
    ...
    if sign_result.get("brainwash_sign_detected"):
        queries.append("brainwash attack continual learning accuracy collapse")
    if sign_result.get("accumulative_sign_detected"):
        queries.append("accumulative poisoning attack streaming gradual decline")
    if sign_result.get("clean_label_sign_detected"):
        queries.append("clean-label poisoning bullseye polytope targeted misclassification")
    if sign_result.get("label_flip_sign_detected"):
        queries.append("label flip poisoning noisy labels training-time")
    if sign_result.get("backdoor_sign_detected"):
        queries.append("backdoor trojan trigger ASR high clean accuracy")
    if sign_result.get("gradient_matching_sign_detected"):
        queries.append("gradient matching data poisoning")
    if not queries:
...
def evaluate_summary_with_ragas(question: str, contexts: List[str], answer: str):
    ...
    - 사용 지표: faithfulness(근거 충실도), answer_relevance(질문 연관도)
    """
    try:
    ...
    try:
        ds = Dataset.from_dict({
            ...
            "answer":        [answer],
            # ground_truths: 없으므로 생략 (있으면 더 많은 지표 가능)
        })
        # llm은 위에서 정의한 ChatOpenAI 인스턴스 재사용
        result = ragas_evaluate(ds, metrics=[faithfulness, answer_relevance], llm=llm)
        try:
            df = result.to_pandas()
            print("\n[RAGAS] 평가 결과")
...
def main():
    ...
    # =========================
    # 질문 정의
    question_text = "주어진 로그와 힌트를 바탕으로 공격 여부·근거·poisoning 여부·공격 패밀리를 정확히 요약하라."
