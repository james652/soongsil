...
def cope_smnist(override_args=None):
    ...
    print(f"\n{benchmark.n_experiences} batches in online data incremental setup.")
...
if __name__ == '__main__':
    ...
    # 결과를 깔끔하게 출력
    print("\n==== Evaluation Results ====")
    print(f"Top-1 Accuracy on each experience:")
    for exp_key in sorted(res.keys()):
        if 'Exp' in exp_key:
            print(f"{exp_key}: {res[exp_key]:.4f}")

    print("\n==== Stream Performance ====")
    print(f"Top-1 Accuracy (Stream): {res.get('Top1_Acc_Stream/eval_phase/test_stream/Task000', 'N/A'):.4f}")
    print(f"Loss (Stream): {res.get('Loss_Stream/eval_phase/test_stream/Task000', 'N/A'):.4f}")
    print(f"Stream Forgetting: {res.get('StreamForgetting/eval_phase/test_stream', 'N/A'):.1f}")
