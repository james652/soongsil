...
def generate_split_cifar100_tasks(task_num, seed=0, rnd_order=True, order=None ):
    ...
    tasks_cls = []
    #cifar_root_path = '/home/jun/work/Brainwash/data'

    cls_per_task = 100 // task_num  
    for i in range(task_num):
        tasks_cls.append(rnd_cls_order[i*cls_per_task:(i+1)*cls_per_task])
    cifar_root_path = '/home/jun/work/soongsil/Agent/data'
    ds_train = CIFAR100(root=cifar_root_path, train=True, download=False, transform=transforms.ToTensor())
    ds_tst = CIFAR100(root=cifar_root_path, train=False, download=False, transform=transforms.ToTensor())
...
def generate_split_cifar10_tasks(task_num, seed=0, rnd_order=True, order=None):
    ...
    tasks_cls = []
    cls_per_task = 10 // task_num   # 예: task_num=5 → 2클래스씩

    for i in range(task_num):
        tasks_cls.append(rnd_cls_order[i*cls_per_task:(i+1)*cls_per_task])

    cifar_root_path = '/home/jun/work/soongsil/Agent/data'  # CIFAR100과 맞춤
    ds_train = CIFAR10(root=cifar_root_path, train=True, download=False, transform=transforms.ToTensor())
    ds_tst = CIFAR10(root=cifar_root_path, train=False, download=False, transform=transforms.ToTensor())
...
def get_dataset_specs(**kwargs):
    ...
    if kwargs['dataset'] == 'split_cifar100':  
        order = np.arange(100)  
        ds_dict, task_order = generate_split_cifar100_tasks(
        ...
        ds_dict, task_order = generate_split_tiny_imagenet_tasks(
            task_num=kwargs['task_num']+1,
            rnd_order=False,
            save_data=False,
            dataset_file='data/tiny_imagenet.npz',
            order=order,
            root_add=tiny_root_add
        )
    ...
    elif kwargs['dataset'] == 'split_cifar10':
        order = np.arange(10)
        ds_dict, task_order = generate_split_cifar10_tasks(
            task_num=kwargs['task_num']+1,   # lasttask=4 → 실제 task 수 = 5
            seed=kwargs['seed'],
            order=order,
            rnd_order=False
        )
        im_sz = 32
        class_num = 10 // (kwargs['task_num']+1)  # 예: 10 // 5 = 2
        emb_fact = 1

    else:
        raise ValueError(f"Unknown dataset: {kwargs['dataset']}")
